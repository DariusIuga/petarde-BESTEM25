...
PLAYER_WORDS_STRING = json.dumps({name: data['cost'] for name, data in PLAYER_WORDS_DATA.items()}, indent=2)
...
METAPROMPT_DEBUG = METAPROMPT_BASE + """
...
def get_llm_choice(system_word: str) -> str:
    ...
    try:
        ...
        if chosen_word_name in PLAYER_WORDS_DATA:
            ...
            potential_matches = [name for name in PLAYER_WORD_NAMES if name.lower() == chosen_word_name.lower()]
            if len(potential_matches) == 1:
                 corrected_name = potential_matches[0]
                 print(f"LLM response '{chosen_word_name}' corrected to '{corrected_name}'.")
...
1.  **`PLAYER_WORDS_STRING`:** A JSON string representation of the word costs is created near the top.
2.  **Metaprompt Refactoring:**
    *   A `METAPROMPT_BASE` string now holds the common parts (word list, rules, general task).
    *   The `PLAYER_WORDS_STRING` is embedded within `METAPROMPT_BASE`.
    *   Instructions were added to **Memorize** the list and **Prioritize Winning** over just low cost, explaining the rationale (high penalty, win discount).
    *   `METAPROMPT` and `METAPROMPT_DEBUG` are now constructed by appending their specific output format instructions to `METAPROMPT_BASE`.
3.  **`get_llm_choice` Update:**
    *   The `prompt` variable no longer includes the f-string `{ {name: data['cost'] for name, data in PLAYER_WORDS_DATA.items()} }`. It now instructs the LLM to use the *memorized* list.
4.  **Minor Correction in Validation:** Added a simple case-insensitive check and correction attempt in the validation logic within `get_llm_choice` in case the LLM returns the word with slightly different capitalization but is otherwise correct.

Now, the word list is sent only once as part of the system prompt (metaprompt), and the LLM is explicitly told to remember it and prioritize winning choices.# filepath: /home/vercingetorix/Documents/BESTEM25/petarde-BESTEM25/strategy.py
import json
import random
import re  # Import regex module for parsing
from time import sleep
