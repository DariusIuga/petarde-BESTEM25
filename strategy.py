...
4.  Choose the *single best word* from the `PLAYER_WORDS_DATA` list that balances effectiveness (likelihood of winning) and cost efficiency according to the game rules.
...
def what_beats(system_word: str) -> int:
    ...
    if chosen_word_id:
        ...
        # Fallback if lookup fails after LLM choice (shouldn't happen with validation)
        print(
            f"Could not find ID for chosen word '{chosen_word_name}'. Choosing random."
        )
        return random.randint(1, 77)
